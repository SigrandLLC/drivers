...
sg16_start_xmit( struct sk_buff *skb, struct net_device *dev )
{
	...
	PDEBUG(xmit_debug,"start");
	if ( !netif_carrier_ok(dev) ){
		PDEBUG(xmit_debug,"carrier fail");
		return 1;
	}
	...
	if( nl->tail_xq == ((nl->head_xq - 1) & (XQLEN - 1)) ) {
		...
		PDEBUG(xmit_debug,"not enougth space");
		goto  err_exit;
	}
	...
	PDEBUG(xmit_debug,"end");	
	return 0;
err_exit:
	spin_unlock_irqrestore( &nl->lock, flags );
	PDEBUG(xmit_debug,"err end");		
	return  1;
}


static void
free_sent_buffers( struct net_device *dev, int need_wake )
{
	struct net_local  *nl = (struct net_local *) dev->priv;
...
alloc_rx_buffers( struct net_device *dev )
{
	...
	PDEBUG(recv_debug,"start, CTDR=%02x LRDR=%02x",nl->regs->CRDR,nl->regs->LRDR);
	...
	PDEBUG(recv_debug,"end, CTDR=%02x LRDR=%02x",nl->regs->CRDR,nl->regs->LRDR);
}
...
indicate_frames( struct net_device *dev )
{
	...
	while( nl->head_rdesc != cur_rbd ) {
		...
		if( len < ETHER_MIN_LEN )
			...
			PDEBUG(0,"(ERR): len=%d too big",len);
//			goto next_pkt;			
		}
