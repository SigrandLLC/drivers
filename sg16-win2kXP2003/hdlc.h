...
struct HldcRegs {               // HLDC registers
  ...
  BYTE TestStatusBits (BYTE const Mask) const { return BYTE (SR & Mask); }
  void ResetStatusBits (BYTE const Mask) { SR = Mask; }

  void SetIntMask (BYTE const Mask) { IMR = Mask; }
  void EnableInts (BYTE const Mask) { SetBits (IMR, Mask); }
  void DisableInts (BYTE const Mask) { ResetBits (IMR, Mask); }

  BYTE GetCTDR (void) const { return CTDR /* % NUM_HLDC_RING_ELEMS */; }
  BYTE GetLTDR (void) const { return LTDR /* % NUM_HLDC_RING_ELEMS */; }
  BYTE GetCRDR (void) const { return CRDR /* % NUM_HLDC_RING_ELEMS */; }
  BYTE GetLRDR (void) const { return LRDR /* % NUM_HLDC_RING_ELEMS */; }

  static UINT NextBD (UINT const BD) { return (BD + 1) % NUM_HLDC_RING_ELEMS; }
...
class DmaBufferDesc {           // DMA buffer descriptor

  public:

  DWORD volatile PhysAddr;      // Physical address
                                // TD: any address
                                // RD: bits 0..1 aren't used
...
class DmaBuffer {               // DMA buffer structure
