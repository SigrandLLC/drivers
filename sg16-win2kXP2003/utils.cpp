...
new( size_t Size )
{
	...
	NDIS_STATUS const	Status=NdisAllocateMemoryWithTag( &Block, Size,
														  *PULONG(Tag) );
	if( Status == NDIS_STATUS_SUCCESS )
	{
		Debug( 2, NULL, "Allocated block %X of %u bytes, total %u", Block, Size,
...
SpinLock::Acquire( void )
{
	if( !IsOwned() )
	{
		...
		NdisGetCurrentProcessorCounts( &Idle, &KUT, &Processor );	/* Processor
																	 * can
																	 * change
																	 * before
																	 * spin
...
/* -----------------------------------------------------------------------------
 *    class DmaPacketDesc functionsConstruction
 ------------------------------------------------------------------------------- */
...
/* class DmaQueue functionsConstruction */
DmaQueue::DmaQueue(
#ifdef _DEBUG
AdapterDesc const *const A
#else
...
/* -----------------------------------------------------------------------------
 *    Create and add new element Creates a descriptor pointed to given memory
 *    block and packet desc
 ------------------------------------------------------------------------------- */
DmaPacketDesc *
DmaQueue::CreateElem( PVOID VA, DWORD PA, PNDIS_PACKET Pkt )
{
	...
	do
	{
		...
		if( !DPD )
		{
			Debug( 9, NULL, "Cannot create DMA packet desc for DmaQueue %X",
				   this );
			break;
		}
...
/* -----------------------------------------------------------------------------
 *    Destroy the queue Deletes all queue elements
 ------------------------------------------------------------------------------- */
void
DmaQueue::DestroyAll( void )
{
	Check();
	while( !IsEmpty() )
	{
 ...
 *    Append element to the queue
 ------------------------------------------------------------------------------- */
void
DmaQueue::Append( DmaPacketDesc *const Elem )
{
...
/* class PacketList functionsConstruction */
 ...
 *    Append packet to the list
 ------------------------------------------------------------------------------- */
void
PacketList::Append( NDIS_PACKET *const Pkt )
{
	Assert( Adapter->IsLocked() );
	Check();
