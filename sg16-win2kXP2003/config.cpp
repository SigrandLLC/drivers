...
NDIS_STATUS AdapterDesc::ReadConfiguration (NDIS_HANDLE ConfigHandle) {
...
#define INT_DATA(Parm) (Parm->ParameterData.IntegerData)
  ...
  NdisReadPciSlotInformation (
    DriverHandle,
    SlotNumber,
    PCI_MEM_BASE_REGISTER,
    &MemoryWindowPhysAddr,
    4
  );
...
/* ----------- End --------------------------------*/
  ...
  if (!*PDWORD (MACAddress) && !*PWORD (MACAddress + 4)) {
    ...
    *PWORD (MACAddress + 4) = WORD (rand ());
    ...
    if (Status != NDIS_STATUS_SUCCESS) {
      ...
      Debug (9, this, "Cannot write random MAC address to registry (%X)", Status);
  ...
  Debug (
    7,
    this,
    "Adapter: MAC=%02X-%02X-%02X-%02X-%02X-%02X",
    MACAddress [0],
    MACAddress [1],
    MACAddress [2],
    MACAddress [3],
  ...
  Debug (
    7,
    this,
    "HLDC: CRC%s, Fill=%s, Pol=%s, RB=%s, WB=%s, XQL=%u, RQL=%u",
    HdlcCfg.crc16? "16" : "32",
    HdlcCfg.fill_7e? "7E" : "FF",
    HdlcCfg.inv? "Neg" : "Pos",
    HdlcCfg.rburst? "On" : "Off",
    HdlcCfg.wburst? "On" : "Off",
    XmtQueueLen, RcvQueueLen
  );

  Debug (
    7,
    this,
    "Modem: Rate=%u, %s, Mod=TCPAM%u, AC=%s",
    ModemCfg.rate,
    ModemCfg.master? "Master" : "Slave",
    4 << (3 - ModemCfg.mod),
...
inline NDIS_STATUS AdapterDesc::MiniportQueryInformation (
  NDIS_OID Oid,
  ...
  switch (Oid) {
    ...
    case OID_GEN_MEDIA_CONNECT_STATUS:
      ...
      Data.U32 = (ModemCfg.AlwaysConnected || ModemState == ACTIVE)? (
    ...
    */
    ...
    case OID_PRIVATE_READ_STAT: { // internal statistics read - start

      if (InformationBufferLength >= 4 && *PDWORD (InformationBuffer) == StatKey) {
    ...
    case OID_PRIVATE_CLEAR_STAT: { // clear statistics
...
inline NDIS_STATUS AdapterDesc::MiniportSetInformation (
  NDIS_OID Oid,
  PVOID InformationBuffer,
  ULONG InformationBufferLength,
  PULONG BytesRead,
  ...
  *BytesNeeded = 0;
  ...
  switch (Oid) {
    ...
    case OID_802_3_MULTICAST_LIST:
      ...
      } else if (InformationBufferLength > MULTICAST_LIST_SIZE * ETH_LENGTH_OF_ADDRESS) {

        StatusToReturn = NDIS_STATUS_MULTICAST_FULL;

      } else {

        NdisMoveMemory (MulticastList, InformationBuffer, InformationBufferLength);
...
void AdapterDesc::ReadModemStatComplete (bool Success) {
...
void AdapterDesc::ResetModemStatComplete (bool Success) {
