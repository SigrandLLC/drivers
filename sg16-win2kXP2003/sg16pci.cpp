...
/* -----------------------------------------------------------------------------
 *    Issue a hardware reset to the NIC and/or reset the driver's software
 *    state
 ------------------------------------------------------------------------------- */
inline NDIS_STATUS
AdapterDesc::MiniportReset(PBOOLEAN need_set_info)
{
	...
	/*
	 * !!!Unlock (); Indicate (NDIS_STATUS_MEDIA_DISCONNECT);!!!Lock ();
	 * ModemState DOWN;
	 */
	CancelTxPackets();
	CancelRxPackets();
...
/* -----------------------------------------------------------------------------
 *    restores a NIC to its initial state when the system is shut down, whether
...
AdapterDesc::MiniportCheckForHang(void)
{
	...
	/*
	 * Lock (); if (ModemState != ACTIVE) { drop_queues (); } Unlock ();
	 * Debug (0, this, "MiniportCheckForHang:EXIT"); return FALSE;
	 */
}
