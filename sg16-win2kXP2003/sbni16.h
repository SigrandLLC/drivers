...
class AdapterDesc {             // Main adapter structure

  public:

  NDIS_HANDLE DriverHandle;     // Miniport handle

  DWORD MemoryWindowPhysAddr;   // Adapter memory window physical address
  PVOID MemoryWindowAddr;       // Virtual address

  HldcRegs *HLDC;               // HLDC register set address
  DmaBuffer *TxDmaBuffer;       // TBD area address
  DmaBuffer *RxDmaBuffer;       // RBD area address
  cx28975_cmdarea *cmdp;        // Modem command window address

  bool MapRegsAllocated;        // Tracking flag

  NDIS_HANDLE PacketPool;       // Received packet pool handle
  NDIS_HANDLE BufPool;          // Received packet buffers pool handle
  ...
  NDIS_EVENT cx_intr;           // Be used while waiting for interrupt

  // Modem command completion routine prototype
  // If called with Arg = DWORD_PTR (-1), an error was occured

  typedef void (AdapterDesc::*ModemCompletionRoutine) (DWORD_PTR Arg);

  ModemCompletionRoutine volatile ModemComplRtn; // Modem command completion routine
  DWORD volatile ModemComplRtnArg; // Completion routine argument
  ...
  PVOID DmaMemAddr;             // Locked memory used for DMA buffers
  NDIS_PHYSICAL_ADDRESS DmaMemPhysAddr; // Physical address
  ULONG DmaMemSize;             // DMA memory size

  DmaQueue XmtDmaPool;          // Transmit DMA packet pool (available desc list)
  DmaQueue XmtDmaQueue;         // Transmit DMA packet queue (active desc list)
  UINT XmtQueueLen;             // Length in packets
  UINT TxRingRemovePos;         // HLDC tx ring remove position

  DmaQueue RcvDmaPool;          // Receive DMA packet pool (available desc list)
  DmaQueue RcvDmaQueue;         // Receive DMA packet queue (active desc list)
  UINT RcvQueueLen;             // Length in packets
  UINT RxRingRemovePos;         // HLDC rx ring remove position

  PacketList PacketsFromNdis;   // Primary queue for holding packets submitted by NDIS
  ...
  void Lock (void) { SL.Acquire (); }
  void Unlock (void) { SL.Release (); }
  bool IsLocked (void) const { return SL.IsOwned (); }
  ...
  bool DoModemCmd (
    ...
    PCVOID Data = NULL,
    UINT Size = 0,
    ModemCompletionRoutine ComplRtn = NULL,
    DWORD_PTR RtnArg = 0
  );
  ...
  void DisableInterrupts (void) { HLDC->SetIntMask (0); }

  void EnableInterrupts (void) {

    HLDC->SetIntMask (EXT | RXS | TXS | OFL | UFL);
  ...
  void Check (void) const {
    ...
    if (DmaMemAddr) {
      ...
      Assert (XmtQueueLen <= NUM_HLDC_RING_ELEMS && TxRingRemovePos < NUM_HLDC_RING_ELEMS);

      Assert (RcvQueueLen <= NUM_HLDC_RING_ELEMS && RxRingRemovePos < NUM_HLDC_RING_ELEMS);
