...
AdapterDesc::CreatePools(void)
{
	...
	do
	{
		...
		/*
		 * NdisAllocatePacketPoolEx (&Status, &PacketPool,
		 * NUMBER_OF_MAP_REGS, 2, 16);
		 */
		if(Status != NDIS_STATUS_SUCCESS)
		{
			Debug(9, this,
				  "MiniportInitialize:ERROR - NdisAllocatePacketPool (%X)", Status);
			break;
		}
		...
		{
			Debug(9, this,
				  "MiniportInitialize:ERROR - NdisAllocateBufferPool (%X)", Status);
			break;
		}
		...
		for(i = 0; i < RcvQueueLen; i++)
		{
			...
			PNDIS_BUFFER	Buf;
			NdisAllocateBuffer(&Status, &Buf, BufPool, VA, ETHERNET_MAX_SIZE);
			Assert(Status == NDIS_STATUS_SUCCESS);
 ...
 *    Initialize the DMA/interrupt support
 ------------------------------------------------------------------------------- */
NDIS_STATUS
AdapterDesc::InitDmaAndInt(void)
{
	...
	do
	{
		...
		Status = NdisMMapIoSpace(&MemoryWindowAddr, DriverHandle, PhysAddr,
								 SG16_MEMSIZE);
		if(Status != NDIS_STATUS_SUCCESS)
		{
			Debug(9, this, "RegisterAdapter:ERROR - NdisMMapIoSpace (%X)",
				  Status);
			break;
		}
		...
		Status = NdisMAllocateMapRegisters(DriverHandle, 0, NDIS_DMA_32BITS, 1,
										   DmaMemSize);
		if(Status != NDIS_STATUS_SUCCESS)
		{
		...
		NdisMAllocateSharedMemory(DriverHandle, DmaMemSize, TRUE, &DmaMemAddr,
								  &DmaMemPhysAddr);
		if(!DmaMemAddr)
		{
		...
		Debug(7, this, "Allocated %u bytes of DMA mem (virt %X, phys %X)",
			  DmaMemSize, DmaMemAddr, DmaMemPhysAddr);
		Status = CreatePools();
		if(Status != NDIS_STATUS_SUCCESS)
		{
			break;
		...
		Status = NdisMRegisterInterrupt(&Interrupt, DriverHandle,
										InterruptNumber, InterruptNumber, TRUE,
										TRUE, NdisInterruptLevelSensitive);
		if(Status != NDIS_STATUS_SUCCESS)
		{
			Debug(9, this, "Cannot register interrupt (%X)", Status);
			NdisWriteErrorLogEntry(DriverHandle,
 ...
 *    Terminate the DMA/Interrupt support
 ------------------------------------------------------------------------------- */
void
AdapterDesc::TermDmaAndInt(void)
{
	...
	if(DmaMemAddr)
	{
		Debug(7, this, "Freeing rcv DMA mem (virt %X, phys %X)", DmaMemAddr,
			  DmaMemPhysAddr);
		NdisMFreeSharedMemory(DriverHandle, DmaMemSize, FALSE, DmaMemAddr,
							  DmaMemPhysAddr);
		DmaMemAddr = NULL;
	}
 ...
 *    Initialization
 ------------------------------------------------------------------------------- */
NDIS_STATUS
AdapterDesc::Init(NDIS_HANDLE MPH, NDIS_HANDLE WrapperConfigurationContext)
{
	DriverHandle = MPH;
