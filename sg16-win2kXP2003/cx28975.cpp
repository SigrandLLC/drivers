...
//
// Modified to C++ by Eugene Muzychenko
//
...
bool AdapterDesc::WaitForModemReply (void) {
  ...
  do {
    ...
    if (!NdisWaitEvent (&cx_intr, 3000)) {
      ...
      Debug (9, this, "Modem didn't respond to command %02X", cmdp->in_opcode);
    ...
    if (!Success) {
      ...
      Debug (
        9,
        this,
        "Modem command %02X failed (%02X)",
        cmdp->in_opcode, cmdp->out_ack
      );
...
//
// Issue modem command without waiting
// ===================================
//
// If both Data and Size are zero, one zero data byte is passed
//

void AdapterDesc::SendModemMessage (BYTE Cmd, PCVOID Data, UINT Size) {
...
bool AdapterDesc::DoModemCmd (
  BYTE Cmd,
  PCVOID Data,
  UINT Size,
  ModemCompletionRoutine ComplRtn,
  DWORD_PTR RtnArg
) {
...
void AdapterDesc::cx28975_interrupt (void) {
...
bool AdapterDesc::download_firmware (PCBYTE const img, UINT const img_len) {
...
bool AdapterDesc::start_cx28975 (PCVOID firmw_img, UINT firmw_len) {
  ...
  do {
	...
	Debug (7, NULL,"SG16: settings:\nRate=%d\nmaser=%d\ncfg=%d\nannex=%d\nmod=%d",
				ModemCfg.rate,ModemCfg.master,ModemCfg.remcfg,ModemCfg.annex,ModemCfg.mod
			);
...
void AdapterDesc::ReadModemStat (DWORD_PTR Stage) {
...
void AdapterDesc::ResetModemStat (DWORD_PTR Stage) {
