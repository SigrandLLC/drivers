...
public:
	NDIS_HANDLE				DriverHandle;								/* Miniport
																		 * handle
																		 * */
	DWORD					MemoryWindowPhysAddr;						/* Adapter
																		 * memory
																		 * window
																		 * physical
																		 * address
																		 * */
	PVOID					MemoryWindowAddr;							/* Virtual
																		 * address
																		 * */
	HldcRegs*				HLDC;										/* HLDC
																		 * register
																		 ...
																		 * address
																		 * */
	DmaBuffer*				TxDmaBuffer;								/* TBD
																		 * area
																		 * address
																		 * */
	DmaBuffer*				RxDmaBuffer;								/* RBD
																		 * area
																		 * address
																		 * */
	cx28975_cmdarea*		cmdp;										/* Modem
																		 * command
																		 * window
																		 * address
																		 * */
	bool					MapRegsAllocated;							/* Tracking
																		 * flag
																		 * */
	NDIS_HANDLE				PacketPool;									/* Received
																		 * packet
																		 * pool
																		 * handle
																		 * */
	NDIS_HANDLE				BufPool;									/* Received
																		 * packet
																		 * buffers
																		 * pool
																		 * handle
																		 * */
	bool					PacketPoolAllocated, BufferPoolAllocated;	/* Tracking
																		 * flags
																		 * */
	...
	NDIS_EVENT				cx_intr;					/* Be used while
														 * waiting for
														 * interrupt */
	...
	ModemCompletionRoutine volatile ModemComplRtn;		/* Modem command
														 * completion routine */
	DWORD volatile					ModemComplRtnArg;	/* Completion routine
														 * argument */
	PVOID							QI_InfoBuffer;		/* QueryInformation
														 * buffer address */
	PULONG							QI_BytesWritten;	/* BytesWritten
														 * reference */
	PULONG							QI_BytesNeeded;		/* BytesNeeded reference */
	PVOID							DmaMemAddr;			/* Locked memory used
														 * for DMA buffers */
	NDIS_PHYSICAL_ADDRESS			DmaMemPhysAddr;		/* Physical address */
	ULONG							DmaMemSize;			/* DMA memory size */
	DmaQueue						XmtDmaPool;			/* Transmit DMA packet
														 * pool (available desc
														 * list) */
	DmaQueue						XmtDmaQueue;		/* Transmit DMA packet
														 * queue (active desc
														 * list) */
	UINT							XmtQueueLen;		/* Length in packets */
	UINT							TxRingRemovePos;	/* HLDC tx ring remove
														 * position */
	DmaQueue						RcvDmaPool;			/* Receive DMA packet
														 * pool (available desc
														 * list) */
	DmaQueue						RcvDmaQueue;		/* Receive DMA packet
														 * queue (active desc
														 * list) */
	UINT							RcvQueueLen;		/* Length in packets */
	UINT							RxRingRemovePos;	/* HLDC rx ring remove
														 * position */
	PacketList						PacketsFromNdis;	/* Primary queue for
														 * holding packets
														 * submitted by NDIS */
...
#ifdef _DEBUG
	LONG volatile					IntDepth;			/* Prevents interrupt
														 * handler reentrance */
	LONG volatile					InCompleteTxDepth;	/* Prevents
														 * CompleteTxPackets
														 * reentrance */
	LONG volatile					InCompleteRxDepth;	/* Prevents
														 * CompleteRxPackets
														 * reentrance */
	ULONG							TotalReceivedBytes; /* Total received bytes
														 * counter */
#endif
	BYTE volatile					LastIntEvents;		/* Status event bits at
														 * last interrupt */
	ModemStates volatile			ModemState;			/* State of the modem */
	BYTE volatile					RdStatStage;		/* Read statistics
														 * stage number */

	/* Current MAC address, multicast address list and filtering mode */
	BYTE							MACAddress[ETH_LENGTH_OF_ADDRESS];
	BYTE							MulticastList[MULTICAST_LIST_SIZE][
